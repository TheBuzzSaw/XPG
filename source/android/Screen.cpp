...
namespace XPG
{
    ...
    {
        ScreenMeta* meta = (ScreenMeta*)app->userData;
        Screen* screen = meta->screen;
    ...
    int32_t GetInput(struct android_app* app, AInputEvent* event)
    {
        ScreenMeta* meta = (ScreenMeta*)app->userData;
        Screen* screen = meta->screen;
    ...
    Screen::Screen()
    {
        ...
        meta->sensorEventQueue = ASensorManager_createEventQueue(
            meta->sensorManager, meta->app->looper, LOOPER_ID_USER, NULL, NULL);
    }
    ...
    void Screen::Open()
    {
        ...
        eglGetConfigAttrib(meta->display, config, EGL_NATIVE_VISUAL_ID, &format);

        ANativeWindow_setBuffersGeometry(meta->app->window, 0, 0, format);

        meta->surface = eglCreateWindowSurface(meta->display, config,
            meta->app->window, NULL);
        ...
        meta->context = eglCreateContext(meta->display, config,
            EGL_NO_CONTEXT, contextAttributes);

        if (eglMakeCurrent(meta->display, meta->surface, meta->surface,
            meta->context) == EGL_FALSE)
        {
        ...
        EGLint w;
        EGLint h;
        eglQuerySurface(meta->display, meta->surface, EGL_WIDTH, &w);
        ...
        rd->width = meta->width;
        rd->height = meta->height;
        meta->screen->OnLoad().Fire(details);
    }
    ...
    void Screen::Close()
    {
        ...
        Event::Details details;
        details.source = this;
        meta->screen->OnUnload().Fire(details);
    ...
    void Screen::Run()
    {
        ...
        while (1)
        {
            ...
            while ((ident=ALooper_pollAll(meta->animating ? 0 : -1, NULL, &events,
                (void**)&source)) >= 0)
            {
                if (source != NULL)
                {
                    source->process(meta->app, source);
                }
